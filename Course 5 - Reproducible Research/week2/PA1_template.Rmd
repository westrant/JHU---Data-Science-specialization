...
The mean is `r mean(totalSteps, na.rm=TRUE)` and the median is `r median(totalSteps, na.rm=TRUE)`.
...
containing the maximum number of steps is `r maxSteps[1]` with `r maxSteps[2]` steps.
...
Overall there are  `r missingData` incomplete rows containing NAs, which leaves `r completeData` complete rows.
...
The mean with NA's replaced is `r mean(totalSteps, na.rm=TRUE)` 
and the median with NAs replaced is `r median(totalSteps, na.rm=TRUE)`.
